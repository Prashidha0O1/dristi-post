    ...
    <Box>
      ...
      <Box border="1px solid var(--color-border)" borderRadius="4px" overflow="hidden" bg="var(--color-surface)">
        ...
        <SimpleGrid columns={7} px="4px" pt="8px" pb="4px">
          {dayHeaders.map((d, i) => (
            <Text key={d} textAlign="center" fontSize="11px" fontWeight="700" color={i === 6 ? BRAND : "var(--color-muted)"} py="4px">
              {d}
            </Text>
          ))}
        </SimpleGrid>
          ...
          {cells.map((day, i) => {
            ...
            return (
              <Flex
                ...
                color={isToday ? "white" : isSaturday ? "#dc2626" : "var(--color-body)"}
                fontWeight={isToday ? "700" : "500"}
                fontSize="13px"
                transition="background 0.1s"
                _hover={day ? { bg: isToday ? BRAND : "var(--color-card-alt)" } : {}}
              >

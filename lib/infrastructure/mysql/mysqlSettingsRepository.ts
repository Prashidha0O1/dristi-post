...
export class MysqlSettingsRepository implements SettingsRepository {
  ...
  async getPoliciesSettings(): Promise<PoliciesSettings> {
    const [rows] = await getPool().query<SettingRow[]>("SELECT payload FROM site_settings WHERE setting_key = 'policies'");
    if (rows.length === 0) return DEFAULT_POLICIES;
    return rows[0].payload as PoliciesSettings;
  }
  ...
  async savePoliciesSettings(settings: PoliciesSettings): Promise<void> {
    await getPool().execute(
      "INSERT INTO site_settings (setting_key, payload) VALUES ('policies', ?) ON DUPLICATE KEY UPDATE payload = VALUES(payload)",
      [JSON.stringify(settings)]
    );
  }
}
